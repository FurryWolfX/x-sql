...
class Builder {
  ...
  readXmlToCache() {
    const fileList = fs.readdirSync(this.dir);
    fileList.forEach(file => {
      const xml = fs.readFileSync(path.resolve(this.dir, file), {
        encoding: "utf-8",
      });
          ...
          if (query.name === "query" && query.attributes.name) {
            query.children.forEach(child => {
              if (child.type !== "text" && child.name !== "if" && child.name !== "for") {
                throw new Error("not a valid query define in " + file);
              }
            });
          } else {
            throw new Error("not a query define in " + file);
          }
        });
        this.cache[root.attributes.namespace] = root.children;
      } else {
        throw new Error("root or namespace not found in " + file);
      }
    });
  }
      ...
      if (query.name === "query" && query.attributes.name === queryName) {
        query.children.forEach(child => {
          if (child.type === "text") {
            ...
            if (sandbox.condition) {
              sql.push(child.children[0].value);
            }
          } else if (child.name === "for" && child.children && child.children[0].type === "text") {
            // 处理 for 相关的逻辑
            const seperator = child.attributes.seperator;
            ...
            sql.push(sqlsInFor.join(seperator));
          } else {
